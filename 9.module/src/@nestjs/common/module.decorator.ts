...
interface ModuleMetadata {
  ...
  providers?: any[];
  exports?: any[]; //模块的导出 可以把自己的一部分providers导出给别的模块的，别的模块只要导入了自己这个模块，就可以使用这一部分provider
...
export function defineModule(nestModule, targets = []) {
  //遍历targets数组，为每个元素添加元数据，key是nestModule,值是对应的模块，用于说明controllers或者providers和nestModule有联系
